...
"""Fail-closed pilot eligibility check for the CBTS coverage tier."""
...
_PILOT_USERS_ENV = "CBTS_COVERAGE_PILOT_USERS"
...
def pilot_users_from_json(raw_value: str) -> frozenset[str]:
    """Parse the Groovy-owned pilot policy, returning an empty set on invalid input."""
    try:
    ...
    for raw_user in payload:
        if not isinstance(raw_user, str) or not raw_user.strip():
            return frozenset()
        pilot_users.append(raw_user.strip())
    return frozenset(pilot_users)


def evaluate_pr_info(pr_info: object, pilot_users: Set[str]) -> tuple[bool, str, str]:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    pilot_users = pilot_users_from_json(os.environ.get(_PILOT_USERS_ENV, ""))
